    ...
    for session in sessions:
        ...
        for fb_doc in feedbacks:
            ...
            if not stats or not focus_type:
                continue
            # スコア・評価算出（get_focus_ratingは(評価, スコア)を返す）
            try:
                rating, score = get_focus_rating(stats, focus_type)
            except Exception as e:
                print(f"Error for session {session_id}, pin {fb_doc.id}: {e}")
                continue
            # Firestoreにscoreとratingを追加
            feedbacks_ref.document(fb_doc.id).update({"score": score, "rating": rating})
            updated_count += 1
            print(f"Updated score/rating for session {session_id}, pin {fb_doc.id}: {score} / {rating}")
